...
for fn in fns:
    ...
    df['altitude'] = df['altitude'] + df['terrain above msl']   # Sets altitude relative to MSL instead of AGL
...
max_traj_height = ds.height[-1]
p_lat_median = ds.lat_median.hvplot(x='time', y='height', cmap='viridis_r', ylim=(0,max_traj_height), clim=(45,85), clabel='Median Latitude', height=300, responsive=True).opts(title='HYSPLIT-5 Back Trajectories for DYE-2, Greenland 2024')
p_alt_min    = ds.alt_min.hvplot(x='time', y='height', cmap='viridis_r', ylim=(0,max_traj_height), clim=(0,max_traj_height), clabel='Minimum Altitude', height=300, responsive=True)
